...
export const botConfig = {
  ...
  presence: {
    ...
    activities: [
      {
        name: "Made with love by Ray", // Required by Discord API
        state: "Mind game",           // What users actually see for Custom status
        type: 4,                      
      },
    ],
  },
...
export function getColor(path, fallback = "#99AAB5") {
  ...
  const parseHex = (val) => (typeof val === "string" && val.startsWith("#") ? parseInt(val.replace("#", ""), 16) : val);

  if (typeof path === "string" && path.startsWith("#")) {
    return parseHex(path);
  }

  const result = path
    .split(".")
    .reduce((obj, key) => (obj && obj[key] !== undefined ? obj[key] : undefined), botConfig.embeds.colors);

  return parseHex(result !== undefined ? result : fallback);
}
...
export function getRandomColor() {
  ...
  return typeof selected === "string" && selected.startsWith("#") ? parseInt(selected.replace("#", ""), 16) : selected;
}
